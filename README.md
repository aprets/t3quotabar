...
- Claude general weekly percentages remaining, without a label: `48% / 80% / 100%`. That is the limit every Claude model draws from; all limits remain in the dropdown.
...
- An `F! 18%` warning appears when Fable, the smaller weekly limit only Fable counts against, drops below 25%. With totals on it averages Fable across accounts, since the balancer sends other models to accounts whose Fable is spent; per account it lists each low account. Accounts within a day of their Fable reset are left out.
