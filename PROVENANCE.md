...
| StatusComponentsMenuView.swift | Sources/CodexBar/StatusComponentsMenuView.swift | Attribution header only. Includes original colors, spacing, fonts and expandable groups. |
| ProviderStatusComponent.swift | Sources/CodexBar/UsageStoreSupport.swift; Sources/CodexBar/UsageStore+Status.swift | Extracted status models and incident.io/Statuspage parsers, moved parser owner to StatusFeed. |
...
Status submenus also copy the hosting relay and deferred height measurement from `StatusItemController+HostedSubmenus.swift`. Public status feeds refresh every five minutes independently of T3 quotas. OpenAI uses its native incident.io feed for groups, falling back to Statuspage components; Claude uses Statuspage components. Missing component data leaves only the status-page link, matching CodexBar. Unlike CodexBar, a failed component refresh clears the old list rather than displaying stale status.
